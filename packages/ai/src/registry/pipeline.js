...
class PipelineExecutor {
        ...
        try {
            ...
            while (remainingSteps.length > 0) {
                ...
                if (parallelSteps.length > 0) {
                    const parallelResults = await Promise.allSettled(parallelSteps.map(step => this.executeStep(step, stepResultsMap, config)));
                ...
                remainingSteps.splice(0, remainingSteps.length, ...remainingSteps.filter(s => !executedSteps.has(s.id)));
            }
    ...
    async executeStep(step, previousResults, config) {
        ...
        try {
            ...
            switch (step.task) {
                case 'reasoning':
                    result = await model.chatCompletion(resolvedInput.messages || []);
                    break;
                case 'embedding':
                    // Use RAG service for embeddings
                    result = await model.generateEmbedding?.(resolvedInput.text);
                    break;
                case 'vision':
                    // Use multimodal model for vision
                    if ('chatWithImage' in model) {
                        result = await model.chatWithImage(resolvedInput.text, resolvedInput.image);
                    }
                    break;
                default:
                    result = await model.chatCompletion(resolvedInput.messages || []);
     ...
     */
    resolveInput(input, previousResults) {
